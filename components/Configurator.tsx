...
export default function Configurator() {
  ...
  return <main className="design">
    ...
    <div className="designGrid">
      ...
      <aside className="live"><SelectedOptionGallery items={selectedItems} empty="이 단계에서 선택한 항목이 사진과 함께 표시됩니다." /><section className="summary"><h2>현재 선택</h2>{bathroomSteps.slice(0, current + 1).flatMap((item) => item.groups.map((group) => { const value=values[group.key]; const first=group.choices.find(choice=>choice.id===(Array.isArray(value)?value[0]:value)); return <div className="summaryRow" key={group.key}>{first&&<BuilderOptionImage option={first} />}<div><span>{group.title}</span><b>{label(value, group.choices)}</b></div></div> }))}</section></aside>
    </div>
