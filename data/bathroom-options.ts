export type Choice={id:string;name:string;sub:string;image:string;builderImage:string};
...
const undecided = (key: BuilderGroupKey, folder: string): Choice => ({
  ...
  image: image(folder), builderImage: builderImage(key, unknownFiles[key] ?? 'unknown'),
});
...
const group = (key: BuilderGroupKey, title: string, folder: string, items: string[], multiple = false): ChoiceGroup => ({
  key, title, multiple, choices: [
    ...items.map(item => {
      ...
      return { id, name, sub, image: image(folder), builderImage: builderImage(key, file) };
    }),
...
export const bathroomSteps:BathroomStep[]=[
 ...
 step('faucet','수전 & 샤워','매일 쓰는 수전과 샤워의 사용감을 정리해보세요.','faucet',[group('faucet','세면 수전','faucet',['one-hole|일반 수전||standard','concealed|매립 세면 수전','tall|볼 세면대용 높은 수전||tall-top-bowl']),group('showerFaucet','샤워 수전','faucet',['shower|일반 샤워수전||standard','bath|욕조 겸용||bathtub-combination','hand-shower|핸드 샤워','concealed-shower|매립 샤워||concealed'],true)]),
 step('drainage','배수 & 구배','배수 방식은 마감 모습과 관리 편의를 함께 살펴보세요.','drainage',[group('drain','배수구','drainage',['square-drain|일반 사각 배수구','tile-drain|타일 삽입형 배수구||tile-insert-drain','linear-drain|라인 배수구','trench-drain|트렌치 드레인']),group('drainPosition','배수 위치','drainage',['keep|현재 위치 유지||keep-position','move|변경 예정||change-position'])]),
